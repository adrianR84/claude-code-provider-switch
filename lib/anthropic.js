...
function fetchAnthropicModels(apiKey = null) {
  return new Promise((resolve, reject) => {
    const cachedModels = modelCache.get(CACHE.KEYS.ANTHROPIC_MODELS);
    if (cachedModels) {
    ...
    if (!apiKey) {
      ...
      apiKey = validateAuthToken(envVars.ANTHROPIC_API_KEY);
    }
    ...
    const options = {
      hostname: ANTHROPIC.DEFAULT_HOST,
      port: ANTHROPIC.DEFAULT_PORT,
      path: ANTHROPIC.API_PATH,
      method: "GET",
      headers: {
        [`${ANTHROPIC.AUTH_HEADER}`]: apiKey,
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          // Cache the response
          modelCache.set(CACHE.KEYS.ANTHROPIC_MODELS, models);
...
async function showModelSelection() {
  ...
  if (!apiKey) {
    ...
    const newApiKey = await promptForApiKey("Anthropic", "ANTHROPIC_API_KEY");
    if (!newApiKey) {
  ...
  try {
    ...
    function displayModels(modelList, showHeader = true, defaultModel = null) {
      ...
      virtualWindow.forEach((model, index) => {
        const maxTokens = model.max_tokens ? ` (${model.max_tokens}K)` : "";
        ...
        log(`${actualIndex}) ${model.id}${maxTokens}${defaultText}`, color);
      });
    ...
    return new Promise((resolve) => {
      ...
      const handleKeyPress = (str, key) => {
        if (key.name === "up") {
          ...
          resolve(DEFAULT_MODELS.ANTHROPIC);
          return;
        }
      };
...
async function launchAnthropic(
  ...
  let selectedModel = envVars.ANTHROPIC_MODEL || DEFAULT_MODELS.ANTHROPIC;

  if (directModel) {
    // Direct model specification - find best match
    log(`Finding best match for model: ${directModel}`, "yellow");
    try {
      const models = await fetchAnthropicModels(envVars.ANTHROPIC_API_KEY);
  ...
  // Add traffic control
  if (envVars.CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC) {
