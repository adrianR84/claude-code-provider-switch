...
class ModelCache {
  ...
  /**
   * Get cache key format used by loadExistingCache and set/get.
   * Matches: "provider-models"  (no colon suffix)
   */
    ...
    files.forEach((file) => {
      if (file.endsWith(".json")) {
        try {
          ...
          // Derive the same cache key that set() uses: "provider-models"
          const key = file.replace(".json", "");
          // Only load entries with a valid timestamp (not corrupted)
