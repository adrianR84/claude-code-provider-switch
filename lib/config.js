...
const { DEFAULT_MODELS, ENV_VARS, DEFAULT_VALUES } = require("./constants");
...
// Import file paths from constants
const { FILE_PATHS } = require("./constants");

// Destructure for convenience
const { claudeDir, globalEnvFile, localEnvFile } = FILE_PATHS;
...
function hasGlobalConfiguration() {
  try {
    ...
    const hasApiKey =
      globalConfig[ENV_VARS.OPENROUTER_AUTH_TOKEN] ||
      globalConfig[ENV_VARS.ANTHROPIC_API_KEY] ||
      globalConfig[ENV_VARS.OLLAMA_AUTH_TOKEN];

    const hasDefaultSetting =
      globalConfig[ENV_VARS.DEFAULT_PROVIDER] ||
      globalConfig[ENV_VARS.DEFAULT_MODEL];
...
async function saveConfigurationLocally() {
  ...
  try {
    ...
    if (fs.existsSync(localEnvFile)) {
      localLog("Local environment file already exists.", "yellow");
    ...
    // Update .gitignore to include local env file
    updateGitignore(localEnvFile);
...
function updateConfigFile(key, value, useGlobal = null) {
  ...
  try {
    ...
    // Update .gitignore if local file
    if (useGlobal === false && targetFile !== globalEnvFile) {
      updateGitignore(targetFile);
    }
...
function updateEnvFile(key, value) {
  try {
    ...
    // Read existing file or create new one
    if (fs.existsSync(globalEnvFile)) {
      content = fs.readFileSync(globalEnvFile, "utf8");
    } else {
      createEnvFile();
      content = fs.readFileSync(globalEnvFile, "utf8");
    }
    ...
    // Write back to file
    fs.writeFileSync(globalEnvFile, updatedLines.join("\n"), "utf8");
...
/**
 * Update .gitignore to include local environment files for security
 */
function updateGitignore(envFilePath) {
  try {
    const { scriptPath } = FILE_PATHS;
    const gitignorePath = path.join(scriptPath, ".gitignore");
    ...
    const lines = content
      ...
      : [];
    const fileExists = lines.some(
      (line) => line.trim() === filename || line.trim() === `# ${filename}`,
    );

    if (!fileExists) {
      lines.push(`# ${filename} - Contains sensitive API keys, keep private`);
      lines.push(filename);
      content = lines.join("\n");
      fs.writeFileSync(gitignorePath, content, "utf8");
    }
  } catch (error) {
    // Don't fail the main operation for gitignore issues
    log(`Warning: Could not update .gitignore: ${error.message}`, "yellow");
  }
}
...
/**
 * Load environment variables from environment file with caching
...
function loadEnvFile() {
  ...
  try {
    ...
    // Merge with process.env to give priority to system environment variables
    const finalConfig = { ...mergedConfig, ...process.env };

    // Atomically update cache
    envCache = finalConfig;
    envCacheTime = now;

    // Include envFile path in the returned object for debugging
    // Point envFile to the actual active configuration source
    finalConfig.envFile = fs.existsSync(localEnvFile)
      ? localEnvFile
      : globalEnvFile;
    finalConfig.localEnvFile = localEnvFile;
...
function getDefaultProvider(envVars = null) {
  ...
  return vars[ENV_VARS.DEFAULT_PROVIDER] || null;
}
...
function getDefaultModel(envVars = null) {
  const vars = envVars || loadEnvFile();
  return vars[ENV_VARS.DEFAULT_MODEL] || "";
}
...
function setDefaultProvider(provider) {
  ...
  updateConfigFile(ENV_VARS.DEFAULT_PROVIDER, provider.trim(), null);
...
function setDefaultModel(model) {
  ...
  updateConfigFile(ENV_VARS.DEFAULT_MODEL, model.trim(), null);
...
function getProviderDefaultModel(provider, envVars = null) {
  const vars = envVars || loadEnvFile();
  const modelMap = {
    openrouter: vars[ENV_VARS.OPENROUTER_MODEL] || DEFAULT_MODELS.OPENROUTER,
    anthropic: vars[ENV_VARS.ANTHROPIC_MODEL] || DEFAULT_MODELS.ANTHROPIC,
    minimax: vars[ENV_VARS.MINIMAX_MODEL] || DEFAULT_MODELS.MINIMAX,
    ollama: vars[ENV_VARS.OLLAMA_MODEL] || DEFAULT_MODELS.OLLAMA,
    original: "",
  };
  ...
  const providers = [
    {
      id: "openrouter",
      name: "OpenRouter",
      envVar: ENV_VARS.OPENROUTER_AUTH_TOKEN,
      message:
        "Please set " + ENV_VARS.OPENROUTER_AUTH_TOKEN + " in environment file",
    },
    { id: "anthropic", name: "Anthropic", envVar: ENV_VARS.ANTHROPIC_API_KEY },
    { id: "minimax", name: "Minimax", envVar: ENV_VARS.MINIMAX_AUTH_TOKEN },
    { id: "ollama", name: "Ollama", envVar: ENV_VARS.OLLAMA_AUTH_TOKEN },
  ];
...
module.exports = {
  colors,
  log,
  envFile: FILE_PATHS.globalEnvFile,
  localEnvFile: FILE_PATHS.localEnvFile,
  globalEnvFile: FILE_PATHS.globalEnvFile,
  claudeDir: FILE_PATHS.claudeDir,
  FILE_PATHS,
  promptForApiKey,
  updateConfigFile,
  loadEnvFile,
