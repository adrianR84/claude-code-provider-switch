...
function showProviderMenu() {
  ...
  // Build providers array dynamically from constants
  const providers = [];
  ...
  function displayMenu() {
    ...
    providers.forEach((provider, index) => {
      ...
      const isClearDefaultsOption = provider.id === "clear-defaults";
      const isSaveLocalOption = provider.id === "save-local";
      const isHelpOption = provider.id === "help";

      // Determine if this is a provider or management option
      const isProvider =
        !isSetDefaultOption &&
        !isApiKeysOption &&
      ...
      // Get icon only for management options
      const getIcon = (id, isProvider) => {
        if (isProvider) {
          return ""; // No icon for providers
        } else {
          switch (id) {
            case "set-default":
              return "⚙️";
      ...
      const icon = getIcon(provider.id, isProvider);
      const aliases =
      ...
      // Enhanced formatting with better spacing and visual elements
      const number = `${index + 1})`.padEnd(4);
      const name = isProvider ? provider.name : `${icon} ${provider.name}`;
      const spacing = isProvider ? "" : " "; // Extra spacing for management options

      log(
        `${marker} ${number} ${spacing}${name}${defaultIndicator}${aliases}`,
        color,
      );

      // Add separator after the actual providers (dynamic count)
      const actualProviderCount = Object.keys(PROVIDERS).length;
      if (index === actualProviderCount - 1) {
        // After the last actual provider
        log(
          "──────────────────────────────────────────────────────────",
          "cyan",
        );
    ...
    // Add helpful usage text with enhanced formatting
    log("🎯 Navigation:", "cyan");
    log("  • Use ↑/↓ arrows to navigate options", "reset");
    log("  • Press Enter to select highlighted option", "reset");
    log(`  • Press 1-${providers.length} for quick select`, "reset");
    log("  • Press ESC to exit", "reset");
    log("", "reset");
    log("⭐ Tips:", "yellow");
    log("  • ⭐ indicates your current default provider", "reset");
    log("  • 🌐 Providers: Launch AI models with different services", "reset");
    log("  • 🔸 Management: Configure settings and preferences", "reset");
    log("", "reset");
  }
