...
const { showModelSelection: showProxyModelSelection } = require("./proxy");
...
async function showModelSelectionForProvider(provider) {
  ...
  switch (provider.id) {
    ...
    case "proxy":
      return await showProxyModelSelection();
  ...
  return new Promise((resolve) => {
    ...
    function display() {
      ...
      options.forEach((option, index) => {
        ...
        console.log(`${marker} ${num} ${icon}${name}\x1b[90m${hint}\x1b[0m\x1b[90m${note}\x1b[0m`);
    ...
    // emitKeypressEvents is idempotent — safe to call each time
...
async function showProviderSubmenu() {
  ...
  Object.keys(PROVIDERS).forEach((providerId) => {
    ...
    if (config && config.aliases && providerId !== "original") {
      const aliasList = config.aliases.join(", ");
      const defaultModel = getProviderDefaultModel(providerId);
      providers.push({
        id: providerId,
        name: getProviderName(providerId),
        hint: aliasList ? ` (${aliasList})` : "",
        note: defaultModel ? ` · ${defaultModel}` : "",
        icon: "🌐",
      });
    }
  });
...
function showProviderMenu() {
  ...
  if (hasDefault) {
    ...
    const title = "--- Default Configuration ---";
    subtitle =
      `\x1b[32m${title}\x1b[0m\n` +
      lines.join("\n") +
      `\n\x1b[32m${"".padEnd(title.length, "-")}\x1b[0m`;
  }
  ...
  const menuOptions = [
    ...
    { id: "claude-code-settings", name: "Inject settings directly into Claude Code settings" },
  ];
  ...
  // Help is always last
  menuOptions.push({ id: "help", name: "Help" });
...
async function setupDefaults() {
  ...
  Object.keys(PROVIDERS).forEach((providerId) => {
    const config = getProviderConfig(providerId);
    if (config && config.aliases && providerId !== "original") {
      const aliasList = config.aliases.slice(1).join(", ");
      const defaultModel = getProviderDefaultModel(providerId);
      providers.push({
        id: providerId,
        name: getProviderName(providerId),
  ...
  try {
    switch (selectedProvider.id) {
      ...
      case "minimax":
        ...
        break;
      case "proxy":
        selectedModel = await showProxyModelSelection();
        break;
      default:
...
function showUsage() {
  ...
  log("  proxy       - Launch Claude Code with local LiteLLM proxy", "reset");
...
/**
 * Read the current env vars from Claude Code settings.json
 */
...
/**
 * Write env vars to Claude Code settings.json (preserves other settings)
 */
function putClaudeCodeSettings(envVars) {
  const settingsPath = getClaudeCodeSettingsPath();
  let settings = {};
  try {
...
/**
 * Remove ANTHROPIC_* keys from Claude Code settings.json env field
 */
function deleteClaudeCodeSettings() {
  const settingsPath = getClaudeCodeSettingsPath();
  let settings = {};
  try {
    settings = JSON.parse(fs.readFileSync(settingsPath, "utf8"));
...
/**
 * Read current env vars from the active .env config and inject them into
 * Claude Code's settings.json env field (ANTHROPIC_BASE_URL, ANTHROPIC_AUTH_TOKEN,
 * ANTHROPIC_MODEL)
 */
function injectClaudeCodeSettings() {
  const { loadEnvFile, getDefaultProvider, getDefaultModel } = require("./config");
  const envVars = loadEnvFile();

  const defaultProvider = getDefaultProvider();
  if (!defaultProvider || defaultProvider === "default" || defaultProvider === "original") {
    log("No default provider set. Set a default first.", "yellow");
    return;
  }
  ...
  // ANTHROPIC_BASE_URL = provider's baseUrl
  toInject["ANTHROPIC_BASE_URL"] = providerConfig.baseUrl;

  // ANTHROPIC_AUTH_TOKEN = provider's auth env var value (e.g. MINIMAX_AUTH_TOKEN)
  const authEnvVar = providerConfig.envVars?.auth;
  if (authEnvVar && envVars[authEnvVar]) {
    toInject["ANTHROPIC_AUTH_TOKEN"] = envVars[authEnvVar];
  }

  // ANTHROPIC_MODEL = default model (from config or provider default)
  const model = getDefaultModel() || DEFAULT_MODELS[defaultProvider];
  if (model) {
    toInject["ANTHROPIC_MODEL"] = model;
  }

  if (Object.keys(toInject).length === 0) {
    log("No ANTHROPIC_* variables found in the active configuration.", "yellow");
    return;
  }

  putClaudeCodeSettings(toInject);
  log("Variables injected into Claude Code settings:", "green");
  Object.entries(toInject).forEach(([k, v]) => log(`  ${k}=${v}`, "yellow"));
}

/**
 * Show the Claude Code settings submenu.
 * Only enabled (has a real option) when at least one ANTHROPIC_* var is set
 * in the active config or already present in settings.json.
 */
async function showClaudeCodeSettingsSubmenu() {
  const { loadEnvFile } = require("./config");
  const envVars = loadEnvFile();

  const hasVarsInConfig =
    envVars[ENV_VARS.ANTHROPIC_BASE_URL] ||
    envVars[ENV_VARS.ANTHROPIC_AUTH_TOKEN] ||
    envVars[ENV_VARS.ANTHROPIC_MODEL];

  const existingInSettings = getClaudeCodeSettings();
  const hasVarsInSettings =
    existingInSettings["ANTHROPIC_BASE_URL"] ||
    existingInSettings["ANTHROPIC_AUTH_TOKEN"] ||
    existingInSettings["ANTHROPIC_MODEL"];

  const options = [];
  if (hasVarsInSettings) {
    options.push({ id: "delete", name: "Delete Variables" });
  }
  if (hasVarsInConfig) {
    options.push({ id: "inject", name: "Inject Variables" });
  }

  if (options.length === 0) {
    log(
      "No ANTHROPIC_* variables to inject or delete.",
      "yellow"
    );
    return;
  }

  // Build subtitle — show current values if already set in settings.json
  let subtitle = "Manage ANTHROPIC_* vars in ~/.claude/settings.json";
  if (hasVarsInSettings) {
    const cur = existingInSettings;
    subtitle =
      `Current in settings.json:\n` +
      `  ANTHROPIC_BASE_URL = ${cur["ANTHROPIC_BASE_URL"] || "(not set)"}\n` +
      `  ANTHROPIC_MODEL    = ${cur["ANTHROPIC_MODEL"] || "(not set)"}\n` +
      `\nManage ANTHROPIC_* vars in ~/.claude/settings.json`;
  }

  return createInteractiveMenu(options, {
