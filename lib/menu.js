...
function showProviderMenu() {
  ...
  const providers = [
    ...
    {
      id: "show-defaults",
      name: "Show Current Defaults",
      aliases: ["show-defaults"],
    },
  ];
...
async function setupDefaults() {
  ...
  log("Available providers:", "yellow");
  providers.forEach((provider, index) => {
    log(`${index + 1}) ${provider.name}`, "reset");
  });
  log("", "reset");

  const providerAnswer = await new Promise((resolve) => {
    rl.question("Select provider (1-4): ", resolve);
  });

  const providerIndex = parseInt(providerAnswer) - 1;
  if (providerIndex < 0 || providerIndex >= providers.length) {
    log("Invalid provider selection", "red");
    rl.close();
    return;
  }
      ...
      switch (selectedProvider.id) {
        case "openrouter":
          ...
          if (!envVars.OPENROUTER_AUTH_TOKEN) {
            ...
            log(
              "Please set OPENROUTER_AUTH_TOKEN in .env file first",
              "yellow",
            );
            log("", "reset");
