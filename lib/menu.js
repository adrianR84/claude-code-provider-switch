...
async function showModelSelectionForProvider(provider) {
  ...
  switch (provider.id) {
    case "openrouter":
      ...
      return await showOpenRouterModelSelection(envVars.OPENROUTER_AUTH_TOKEN);

    case "ollama":
      return await showOllamaModelSelection();

    case "anthropic":
      // Anthropic doesn't have a model API, create a simple selection
      ...
      log("Available Anthropic models:", "yellow");
      log("", "reset");
      log("Selection Options:", "yellow");
      log("• Type the number to select a model from the list", "reset");
      log("• Press Enter to use the default model", "reset");
      log("", "reset");
      ...
      // Show default model first
      if (anthropicModels.includes(defaultModel)) {
        log(`1) ${defaultModel} [DEFAULT]`, "green");
        anthropicModels.splice(anthropicModels.indexOf(defaultModel), 1);
      } else {
        log(`1) ${defaultModel} [DEFAULT]`, "green");
      }
      ...
      anthropicModels.forEach((model, index) => {
        log(`${index + 2}) ${model}`, "reset");
      });

      log("", "reset");

      const answer = await new Promise((resolve) => {
        rl.question("Select model (Enter for default): ", resolve);
      });
      ...
      if (selection === 1) {
        return defaultModel;
      } else if (selection > 1 && selection <= anthropicModels.length + 1) {
        return anthropicModels[selection - 2];
      } else {
        log("Invalid selection, using default model", "yellow");
        return defaultModel;
      }

    case "default":
      return ""; // Default provider has no model selection
...
async function setupDefaults() {
  ...
  const providers = [
    ...
    { id: "default", name: "Default Claude Code" },
  ];
  ...
  if (selectedProvider.id !== "default") {
    ...
    try {
      switch (selectedProvider.id) {
        ...
        case "anthropic":
          // Anthropic doesn't have a model API, use manual input
          log("Anthropic models available (enter manually):", "yellow");
          log("- claude-3-5-sonnet-latest", "reset");
          log("- claude-3-5-haiku-latest", "reset");
          log("- claude-3-opus-latest", "reset");
          log("", "reset");
          const anthropicAnswer = await new Promise((resolve) => {
            modelRl.question(
              `Enter model name (leave empty for ${getProviderDefaultModel(selectedProvider.id)}): `,
              resolve,
            );
