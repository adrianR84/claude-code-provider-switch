...
const { MINIMAX, CACHE, HTTP_STATUS, DEFAULT_MODELS } = require("./constants");
const {
  NetworkError,
  ValidationError,
  AuthenticationError,
...
async function showModelSelection() {
  ...
  try {
    const models = await fetchMinimaxModels(envVars.MINIMAX_AUTH_TOKEN);
    ...
    return new Promise((resolve) => {
      rl.question("Select a model (enter number or model name): ", (answer) => {
        ...
        // Try to find by name
        const matchedModel = findBestMatchingModel(trimmedAnswer, sortedModels);
        if (matchedModel) {
...
async function launchMinimax(
  ...
  if (!envVars.MINIMAX_AUTH_TOKEN) {
    log("Minimax auth token not found. Please provide it:", "yellow");
    const authToken = await promptForApiKey("Minimax", "MINIMAX_AUTH_TOKEN");
    if (!authToken) {
      log("Error: Minimax auth token is required", "red");
      process.exit(1);
    }
    updateConfigFile("MINIMAX_AUTH_TOKEN", authToken, null);
    envVars.MINIMAX_AUTH_TOKEN = authToken;
  }

  let selectedModel = envVars.MINIMAX_MODEL || DEFAULT_MODELS.MINIMAX;
  ...
  env.ANTHROPIC_AUTH_TOKEN = envVars.MINIMAX_AUTH_TOKEN;
