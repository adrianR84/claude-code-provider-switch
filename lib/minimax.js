...
function fetchMinimaxModels(apiKey = null) {
  return new Promise((resolve, reject) => {
    const cachedModels = modelCache.get("minimax-models");
    if (cachedModels) {
    ...
    try {
      ...
      modelCache.set("minimax-models", staticModels);
...
async function launchMinimax(
  ...
  log("Using:", "yellow");
  log(`  Base URL: ${getProviderBaseUrl("minimax")}`, "reset");
  log(`  Model: ${selectedModel}`, "reset");
  log("", "reset");

  // Set environment variables
  const env = { ...process.env };
  env.ANTHROPIC_BASE_URL = getProviderBaseUrl("minimax");
