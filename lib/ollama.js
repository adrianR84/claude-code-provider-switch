...
function fetchOllamaModels(authToken = null) {
  return new Promise((resolve, reject) => {
    ...
    // Validate auth token
    const sanitizedToken = validateAuthToken(authToken);
    ...
    const options = {
      ...
      headers: {
        ...
        ...(sanitizedToken && {
          Authorization: `${OLLAMA.AUTH_HEADER} ${sanitizedToken}`,
        }),
      },
    };
      ...
      res.on("end", () => {
        try {
          ...
          if (error instanceof ValidationError) {
            ...
            reject(
              new NetworkError(
                `Failed to parse Ollama API response: ${error.message}`,
                "ollama",
              ),
            );
          }
    ...
    req.on("error", (error) => {
      reject(
        new NetworkError(`Failed to fetch models: ${error.message}`, "ollama"),
      );
    });
    ...
    req.setTimeout(OLLAMA.TIMEOUT, () => {
      ...
      reject(
        new NetworkError(
          "Request timeout - failed to fetch models from Ollama",
          "ollama",
        ),
      );
    });
...
async function showModelSelection() {
  ...
  const authToken = validateAuthToken(envVars?.OLLAMA_AUTH_TOKEN);
...
async function launchOllama(
  ...
  let authToken = validateAuthToken(envVars.OLLAMA_AUTH_TOKEN);
