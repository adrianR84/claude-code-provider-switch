      ...
      res.on("end", () => {
        try {
          ...
          if (response.models && Array.isArray(response.models)) {
            // Filter for cloud models only (those with :cloud suffix)
            const cloudModels = response.models.filter((model) =>
...
async function showModelSelection() {
  log("Fetching available cloud models from Ollama...", "yellow");

  try {
    const models = await fetchOllamaModels();
    log(
      `Found ${models.length} cloud models. Start typing to filter, or press Enter to select.`,
      "green",
    );
