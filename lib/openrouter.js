...
const { log, loadEnvFile, findBestMatchingModel } = require("./config");
...
// Cache for network failures to avoid repeated failed requests
let networkFailureCache = null;
let networkFailureTime = 0;
const NETWORK_FAILURE_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

/**
 * Check if we should skip network requests due to recent failures
 */
function shouldSkipNetworkRequests() {
  const now = Date.now();
  return (
    networkFailureCache && now - networkFailureTime < NETWORK_FAILURE_CACHE_TTL
  );
}
...
function fetchOpenRouterModels(apiKey = null) {
  return new Promise((resolve, reject) => {
    // Check if we recently had a network failure
    const now = Date.now();
    if (
      networkFailureCache &&
      now - networkFailureTime < NETWORK_FAILURE_CACHE_TTL
    ) {
      reject(networkFailureCache);
      return;
    }
    ...
    if (cachedModels && !Array.isArray(cachedModels)) {
      modelCache.clear(CACHE.KEYS.OPENROUTER_MODELS);
    }
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          const models = validateModelResponse(response);

          // Clear network failure cache on successful request
          networkFailureCache = null;
          networkFailureTime = 0;
    ...
    req.on("error", (error) => {
      let errorMessage = `Failed to fetch OpenRouter models: ${error.message}`;

      // Provide more helpful error messages for common network issues
      if (error.message.includes("ENOTFOUND")) {
        errorMessage = `Network error: Cannot resolve ${OPENROUTER.DEFAULT_HOST}. Please check your internet connection or DNS settings.`;
      } else if (error.message.includes("ECONNREFUSED")) {
        errorMessage = `Connection error: Cannot connect to ${OPENROUTER.DEFAULT_HOST}. The service may be down or blocked by firewall.`;
      } else if (error.message.includes("ETIMEDOUT")) {
        errorMessage = `Connection timeout: Request to ${OPENROUTER.DEFAULT_HOST} timed out. Please check your network connection.`;
      }

      const networkError = new NetworkError(errorMessage, "openrouter");

      // Cache the network failure to avoid repeated requests
      networkFailureCache = networkError;
      networkFailureTime = Date.now();
    ...
    req.setTimeout(OPENROUTER.TIMEOUT, () => {
      ...
      networkFailureCache = timeoutError;
      networkFailureTime = Date.now();
...
async function showModelSelection() {
  const envVars = loadEnvFile();
  const apiKey = validateAuthToken(envVars.OPENROUTER_AUTH_TOKEN);
  ...
  // Check for recent network failures
  if (shouldSkipNetworkRequests()) {
    log(
      "Network error: Cannot resolve api.openrouter.ai. Please check your internet connection or DNS settings.",
      "red",
    );
    log("Cannot fetch models from OpenRouter due to network issues", "yellow");
  ...
  try {
    ...
    if (error instanceof NetworkError) {
      ...
      log(
        "Cannot fetch models from OpenRouter due to network issues",
        "yellow",
      );
    } else if (error instanceof AuthenticationError) {
      log(`Authentication error: ${error.message}`, "red");
      log("Please check your OpenRouter API key", "yellow");
    } else {
...
async function launchOpenRouter(
  ...
  let authToken = validateAuthToken(envVars.OPENROUTER_AUTH_TOKEN);

  if (!authToken) {
    log("OpenRouter auth token not found. Please provide it:", "yellow");
    authToken = await promptForApiKey("OpenRouter", "OPENROUTER_AUTH_TOKEN");
    if (!authToken) {
  ...
  if (directModel && directModel !== DEFAULT_MODELS.OPENROUTER) {
    // Only try to match if it's not the default model
    ...
    if (shouldSkipNetworkRequests()) {
      log(
        `Network error fetching models: Network error: Cannot resolve ${OPENROUTER.DEFAULT_HOST}. Please check your internet connection or DNS settings.`,
        "red",
      );
      log(
        "Continuing with default model - you can still use Claude Code",
        "yellow",
      );
      ...
      try {
        ...
        if (error instanceof NetworkError) {
          log(`Network error fetching models: ${error.message}`, "red");
          log(
            "Continuing with default model - you can still use Claude Code",
            "yellow",
          );
    ...
    // Skip network request if we recently had failures
    if (shouldSkipNetworkRequests()) {
      log(
        `Network error: Cannot resolve ${OPENROUTER.DEFAULT_HOST}. Please check your internet connection or DNS settings.`,
        "red",
      );
      log(
        "Cannot fetch models from OpenRouter due to network issues",
  ...
  log("Using:", "yellow");
  log(`  Base URL: https://${OPENROUTER.DEFAULT_HOST}/api`, "reset");
  log(`  Model: ${selectedModel}`, "reset");
  log("", "reset");

  // Set environment variables
  const env = { ...process.env };
  env.ANTHROPIC_BASE_URL = `https://${OPENROUTER.DEFAULT_HOST}/api`;
