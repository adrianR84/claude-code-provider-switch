...
const { log, loadEnvFile, promptForApiKey, updateEnvFile } = require('./config');
...
function fetchOpenRouterModels(apiKey) {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(10000, () => {
      req.destroy();
      reject(new Error('Request timeout - failed to fetch models from OpenRouter'));
    });
...
async function showModelSelection(apiKey) {
  ...
  try {
    ...
    log(`Found ${models.length} models. Start typing to filter, or press Enter to select.`, 'green');
    ...
    function displayModels(modelList, showHeader = true) {
      ...
      if (modelList.length > 20) {
        log('', 'reset');
        log(`... and ${modelList.length - 20} more models matching your filter`, 'yellow');
      }
    ...
    return new Promise((resolve) => {
      ...
      rl.on('line', (input) => {
        ...
        if (!isNaN(selection) && selection > 0 && selection <= Math.min(filteredModels.length, 20)) {
        ...
        if (currentFilter === '') {
          ...
          filteredModels = models.filter(model => 
            model.id.toLowerCase().includes(currentFilter)
...
async function launchOpenRouter(showModelMenu = false, extraArgs = []) {
  ...
  if (!envVars.OPENROUTER_AUTH_TOKEN) {
    ...
    const authToken = await promptForApiKey('OpenRouter', 'OPENROUTER_AUTH_TOKEN');
    if (!authToken) {
