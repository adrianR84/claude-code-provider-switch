  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      rl.on("line", (input) => {
        ...
        // Check for empty input (Enter key) - select first visible model
        if (trimmedInput === "") {
          if (filteredModels.length > 0) {
            const selectedModel = filteredModels[virtualOffset];
            rl.close();
            log(`Selected model: ${selectedModel.id}`, "yellow");
            resolve(selectedModel.id);
            return;
        ...
        // Redisplay with new filter - don't show header to avoid clearing screen
        displayModels(filteredModels, false, defaultModel);
      });
