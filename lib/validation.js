...
function validateModelName(model) {
  ...
  if (trimmed.length > 100) {
    throw new ValidationError('Model name too long (max 100 characters)', 'model');
  }
...
function validateAuthToken(token) {
  ...
  if (trimmed.length < 10) {
    throw new ValidationError('Auth token too short (min 10 characters)', 'token');
  }
  
  if (trimmed.length > 1000) {
    throw new ValidationError('Auth token too long (max 1000 characters)', 'token');
  }
...
function validateModelResponse(response) {
  ...
  if (!Array.isArray(models)) {
    throw new ValidationError('Invalid response: expected array of models', 'models');
  }
  ...
  // Validate each model has at least a name field
  models.forEach((model, index) => {
    if (!model || typeof model !== 'object') {
      throw new ValidationError(`Invalid model at index ${index}: expected object`, `models[${index}]`);
    }
    
    if (!model.name || typeof model.name !== 'string') {
      throw new ValidationError(`Model at index ${index} missing name field`, `models[${index}].name`);
    }
  });
...
function validateHostname(hostname) {
  ...
  if (trimmed.length > 253) {
    throw new ValidationError('Hostname too long (max 253 characters)', 'hostname');
  }
  
  // Basic hostname validation
  const hostnameRegex = /^[a-zA-Z0-9.-]+$/;
  if (!hostnameRegex.test(trimmed)) {
    throw new ValidationError('Hostname contains invalid characters', 'hostname');
  }
